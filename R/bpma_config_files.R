...
#
###########################################################################
FolderRoot = "~/Best-Partition-MaF1-ECC"
FolderScripts = "~/Best-Partition-MaF1-ECC/R"
...
###############################################################################
FolderCF = paste(FolderRoot, "/bpma-ecc-config-files", sep="")
...
while(f<=length(similarity)){
  ...
  while(i<=n){
    ...
    # Confi File Name
    file_name = paste(FolderS, "/", sim[f], "-e-bpma-", ds$Name, ".csv", sep="")
    ...
    write("Dataset_Path, ~/Best-Partition-MaF1-ECC/Datasets",
          file = output.file, append = TRUE)

    # job name
    job_name = paste(sim[f], "-bpma-", ds$Name, sep = "")
    ...
    str = paste("~/Best-Partition-MaF1-ECC/Partitions/",
                similarity[f], sep="")
